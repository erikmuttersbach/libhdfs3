...
static void BuildResponse(uint32_t id,
                          RpcStatusProto status, const char * errClass,
    ...
    if (errClass != NULL) {
        size = strlen(errClass);
        buffer.writeBigEndian(size);
        buffer.write(errClass, size);
    }

    if (errMsg != NULL) {
        size = strlen(errMsg);
        buffer.writeBigEndian(size);
        buffer.write(errMsg, size);
    }
...
TEST(TestRpcChannel, TestCheckResponse_InvalidCallId) {
    ...
    BuildResponse(0, RPC_SUCCESS, NULL, NULL, NULL, resp);
    RpcChannelKey key = BuildKey();
    MockRpcClient client;
    EXPECT_CALL(client, getClientId()).Times(AnyNumber()).WillRepeatedly(Return(""));
    MockSocket * sock = new MockSocket();
...
TEST(TestRpcChannel, TestCheckResponse_ErrorResponse) {
    ...
    BuildResponse(0, RPC_ERROR, errClass, errMsg, NULL, resp);
    RpcChannelKey key = BuildKey();
    MockRpcClient client;
    EXPECT_CALL(client, getClientId()).Times(AnyNumber()).WillRepeatedly(Return(""));
    MockSocket * sock = new MockSocket();
...
TEST(TestRpcChannel, TestCheckResponse_FatalResponse2) {
    ...
    BuildResponse(0, RPC_FATAL, errClass, errMsg, NULL, resp);
    RpcChannelKey key = BuildKey();
    MockRpcClient client;
    EXPECT_CALL(client, getClientId()).Times(AnyNumber()).WillRepeatedly(Return(""));
    MockSocket * sock = new MockSocket();
...
TEST(TestRpcChannel, TestInvoke_RetryIdempotent) {
    ...
    BuildResponse(callid, RPC_SUCCESS, NULL, NULL, &resp, respBody);
    MockSocket * sock = new MockSocket();
    RpcChannelKey key = BuildKey();
