...
private:
    ...
    /**
     * Send sasl messge to server.
     * @param msg The message send to server.
     */
    void sendSaslMessage(const std::vector<char> & msg);

    /**
     * Receive sasl message from server.
     * @param buffer The output parameter to get the message.
     * @return Return false if the client should switch to SIMPLE method.
     */
    bool getSaslChallenge(std::vector<char> & buffer);

    /**
     * Choose a RpcAuth to use.
     * @param auth Current RpcAuth method used.
     * @return Return the RpcAuth should be used.
     */
    RpcAuth chooseAuthMethod(const RpcAuth & auth);

    /**
     * Setup the SASL connection to server.
     * @param auth The SASL auth information.
     * @return A RpcAuth variable to indicate the method actually used.
     */
    RpcAuth setupSaslConnection(const RpcAuth & auth);

    /**
     * Setup the SASL auth information.
     * @param auth The RPC auth information.
     * @return Return the SASL auth information.
     */
    SaslAuth setupSaslAuth(const RpcAuth & auth) ;
