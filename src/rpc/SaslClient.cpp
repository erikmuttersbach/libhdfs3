...
SaslClient::SaslClient(const SaslAuth & auth, const Token & token,
    ...
    switch (auth.getRpcAuth().getMethod()) {
...
void SaslClient::initKerberos(const SaslAuth & auth,
                              const std::string & principal) {
    int rc;

    /* Create new authentication session. */
    if ((rc = gsasl_client_start(ctx, auth.getMechanism().c_str(), &session)) != GSASL_OK) {
        THROW(HdfsIOException, "Cannot initialize client (%d): %s", rc,
              gsasl_strerror(rc));
    }

    gsasl_property_set(session, GSASL_SERVICE, auth.getService().c_str());
    gsasl_property_set(session, GSASL_AUTHID, principal.c_str());
    gsasl_property_set(session, GSASL_HOSTNAME, auth.getServerHost().c_str());
}
...
void SaslClient::initDigestMd5(const SaslAuth & auth,
                               const Token & token) {
    int rc;
...
std::vector<char> SaslClient::evaluateChallenge(const std::vector<char> & challenge) {
    int rc;
    char * output = NULL;
    size_t outputSize;
