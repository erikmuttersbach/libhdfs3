...
static std::string SetupDatanodePrincipal(const std::string & principal,
        const std::string & host) {
    KerberosName name(principal);

    if (name.getHost() == "_HOST") {
        name.setHost(host);
    }

    return name.getPrincipal();
}

DatanodeImpl::DatanodeImpl(const std::string & host, uint32_t port, const std::string & hostname,
    ...
    server.setPrincipal(SetupDatanodePrincipal(c.getDatanodePrincipal(), hostname));
}
